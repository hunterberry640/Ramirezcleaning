...
#### 2i. `infra/README.md`
- Replace domain references with the new domain.
...
### Step 5 — Optional GitHub Pages staging
...
### Step 2 — AWS account and credentials

Ask the user if they have an AWS account ready. If not:
- Direct to https://aws.amazon.com/ to create one
- Walk through `aws configure` to set Access Key ID, Secret Access Key, and default region

If they already have credentials configured, verify with:
```bash
